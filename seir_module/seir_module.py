...
def read_data_for_country(country, province, directory):
    ...
    return pd.DataFrame({'confirmed': rus_confirmed.values, 
                       ...
                       index=rus_confirmed.index).sort_index()
...
#     return pd.DataFrame({'confirmed': conf_data[conf_data['Country/Region']==country][4:]})
    ...
    return pd.DataFrame({'confirmed': conf_data.values, 
                       ...
                       index=index).sort_index().astype(int)
...
def solve_seir(S0, E0, I0, R0, betas, gamma, alpha, mu, times):
    ...
    y0 = S0, E0, I0, R0
    # print((betas, gamma, alpha, mu))
    ret = solve_ivp(fun=lambda t, y: deriv(t, y, betas, gamma, alpha, mu),
     t_span=(0, times[-1]), y0=y0, t_eval=times)
    # print("OK")
    S, E, I, R = ret.y
    return S, E, I, R
...
def optimize_twostep_beta(data_df, seir_params):   
    params = differential_evolution(lambda x: twostep_beta_cost_func(x[0], x[1], x[2], x[3], x[4], data_df, seir_params), 
...
def train(country_name, data, N, beta_opt_func, dot_number=8):
#     try:
...
def train_all(rus_regions, countries, rus_directory, country_directory, beta_opt_func, dot_number=2):
    rus_data = {}
    ...
    for country_name, country_stat in tqdm(country_data.items(), total=len(country_data)):
        ...
        countries_seir_params[country_name] = train(country_name, country_stat, 
            countries[countries.ccse_name==country_name]['population'].iloc[0],
            beta_opt_func, dot_number=dot_number)
    ...
    for province, data in tqdm(rus_data.items(), total=len(rus_data)):
        ...
        rus_seir_params[province] = train(province, data, 
            rus_regions[rus_regions.csse_province_state==province]['population'].iloc[0],
            beta_opt_func, dot_number=dot_number)
    ...
    for name, seir in countries_seir_params.items():
        ...
        new_subm = pd.DataFrame({
            ...
            'prediction_confirmed': pred.predicted_confirmed,
            'prediction_deaths': pred.predicted_confirmed*0.1
        })
