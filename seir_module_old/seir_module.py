...
def optimize_twostep_beta(data_df, seir_params):   
    params = differential_evolution(
        lambda x: twostep_beta_cost_func(x[0], x[1], x[2], x[3], x[4], data_df, seir_params), 
              bounds=[(0, 1), (0, 1), (0, 1), (0, data_df.shape[0]), (0, data_df.shape[0])], seed=157)
...
def run_train_test(data_df, contry_name, N, cut_koef=10000000000, dot_number=None, test_fold_size=1, beta_opt_func=optimize_zerostep_beta):
...
def train(country_name, data, N, dot_number=10000000000):
#     try:
